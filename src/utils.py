...
def ensemble_predict(models: List[torch.nn.Module], frames: torch.Tensor, device: torch.device) -> torch.Tensor:
    all_probs = []
...
def create_video_from_frames(frames: np.ndarray, output_path: str, fps: int = 10):
...
def save_predictions_csv(predictions: np.ndarray, true_labels: np.ndarray, idx_to_label: Dict, output_path: str):
    import pandas as pd
...
def calculate_per_class_metrics(confusion_matrix: np.ndarray, idx_to_label: Dict):
    import pandas as pd
